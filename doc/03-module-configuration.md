...
The configuration can be done via the web interface or by directly editing the configuration inifile.
...
On most distributions, if you used the repository as an install source, you will find the configuration file in `/etc/icingaweb2/modules/grafana`.
...
|-----------------------|-----------------------|
|host                   | **Required.** Grafana server hostname (and port).|
...
|datasource             | **Required for Grafana 4.x only.** Grafana data source type (`influxdb`, `graphite` or `pnp`). Defaults to `influxdb`.|
...
|authentication         | **Proxy only** Authentication type used to access Grafana. It can be set to `anon`,`token` or `basic`. Defaults to `anon`.
...
|ssl_verifyhost         | **Proxy mode only** **Optional.** Verify the certificate's name against the host. Defaults to `false`.|
...
recommended using a secure protocol (HTTPS).

### height
The default height the graphs use in pixels. Defaults to `280` pixels.
This option can be overwritten by a graph configuration.

### width
The default width the graphs use in pixels. Defaults to `640` pixels.
This option can be overwritten by a graph configuration.

### timerange
The default time range used for the graphs. Defaults to `6 hours`.
This option can be overwritten by a graph configuration.

### timerangeAll
The time range for all graphs feature. Defaults to `Previous week`

### enableLink
Enable or disable the graphs as a link to the Grafana Server.

### datasource
The data source that Grafana server uses. It can be InfluxDB, or Graphite.

### defaultorgid
The number of the default organization id where dashboards are located. Defaults to `1`.
...
#### indirectproxy
The module will still fetch the graphs with curl on **server side**, but the images are given by reference.
This will speed up the page load, but the image will take several seconds to show up.
...
### authentication
Authentication type used to access Grafana. It can be set to `anon`,`token` or `basic`. Defaults to `anon`.
...
### apitoken
API token used to access Grafana. See [Create API Token](http://docs.grafana.org/http_api/auth/#create-api-token) for details on how to create an API token.
...
If you have set your Grafana URL for example to `localhost` then you can set here a URL that is used for external access to link to Grafana.

### publichost
Used with 'usepublic = yes'.
Public hostname and port. Same as the `host` option.
...
### custvardisable
The custom variable name (vars.idontwanttoseeagraph for example) that will disable graphs if set to anything else than false (bool or string). Defaults to `grafana_graph_disable`.
...
### custvarconfig
The custom variable name (vars.usegraphconfig for example) that will be used as a graph config name. Defaults to `grafana_graph_config`.
...
If there is no such configuration, the `default-template` will be used.

### theme
The Grafana theme will be used. Defaults to `light`.

### debug
Show debug information if the user has permission to see them. This defaults to `false`.
